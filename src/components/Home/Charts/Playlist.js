...
const Playlist = () => {
  ...
  useEffect(() => {
    const playlist = data.find((datum) => {
      return datum.id === parseInt(id);
    });
    setChart(playlist);
  }, []);
    ...
    <div className='playlist'>
      ...
      <header>
        <div className='hero-img'>
          <img src={chart.cover2} alt={chart.title} />
        </div>

        <div className='hero-txt'>
          <h1>{chart.title}</h1>
          <p className='mt-2 text-xss'>{chart.narration}</p>
          <p className='font-semibold mb-8'>
            {chart.songs} songs - {chart.duration}
          </p>
