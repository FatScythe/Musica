...
const MusicaProvider = ({ children }) => {
  ...
  let loopContainer = useRef();
  let shuffleContainer = useRef();
  let updateTimer;
  let seekPosition;

  const [coverSrc, setCoverSrc] = useState(img);
  const [artist, setArtist] = useState("artist");
  const [title, setTitle] = useState("Click on a card to play a song");
  ...
  // loop loaded track
  const handleLoop = () => {
    loopContainer.current.classList.toggle("bg-secondary");
    if (loopContainer.current.classList.contains("bg-secondary")) {
      setIsLooped(true);
    } else {
      setIsLooped(false);
    }
  ...
  const loadTrack = (list, index, cover, artist, title) => {
    ...
    if (trackList) {
      ...
      audioContainer.current.addEventListener("ended", () => {
        isLooped ? playTrack() : playNextTrack(listIndex);
      });
    }
  };
  ...
  // Increase and decrease track Volume
  const setVolume = () => {
  ...
  // Track's Shuffles
  const handleShuffle = () => {
    shuffleContainer.current.classList.toggle("bg-secondary");
    if (shuffleContainer.current.classList.contains("bg-secondary")) {
      setIsShuffled(true);
    } else {
      setIsShuffled(false);
    }
  ...
  // Next track
  const nextTrack = () => {
    if (audioContainer.current.src && !isShuffled) {
    ...
    //Shuffles next track
    if (audioContainer.current.src && isShuffled) {
      let random = Math.floor(trackList.length * Math.random());
      if (trackIndex < trackList.length - 1) {
        loadTrack(trackList, random, coverSrc, artist, title);
        setCoverSrc(trackList[random].cover);
        setArtist(trackList[random].artist);
